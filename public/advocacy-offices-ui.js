...
(function advocacyOfficesUiModule(global) {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .ao-kpi { padding: 14px; border-radius: 12px; border: 1px solid var(--border, rgba(148,163,184,.18));
        background: var(--card, #fff); color: var(--text, #0f2744); }
      [data-theme="light"] .ao-kpi { background: #fff; }
      .ao-kpi span { display: block; font-size: .72rem; color: var(--muted, #64748b); text-transform: uppercase; letter-spacing: .04em; }
      .ao-kpi strong { font-size: 1.25rem; color: var(--text, #0f2744); }
  ...
  function renderCadastro() {
    ...
    root.innerHTML =
      `<div class="filter-bar" style="margin:0 0 12px">
        ...
        <input id="aoCadastroSearch" type="search" placeholder="Nome, gestor de carteira, CNPJ, telefone…" value="${esc(_cadastroFilters.search)}" />
      ...
      <div class="table-wrap">
        <table class="table stacked">
          <thead><tr>
            <th>Nome</th><th>CNPJ</th><th>Cidade/Estado</th><th>Telefone</th><th>Gestores de Carteira</th><th>Gestores de Cobrança</th><th>Status</th><th>Ações</th>
          </tr></thead>
          <tbody>${
            list.length
              ? list
                  .map((o) => {
                    ...
                    const pm = global.__ampliState?.partnerManagers || [];
                    const officeMgrs = svc().managersForOffice(global.__ampliState?.advocacyOfficeManagers || [], o.id);
                    const cobr = mgrSvc ? mgrSvc.countsForOffice(pm, o.id).cobranca : 0;
                    ...
                    const toggle = o.active ? "Inativar" : "Reativar";
                    const del = n
                    ...
                    return `<tr>
                      ...
                      <td data-label="Carteira">${officeMgrs.length}</td>
                      <td data-label="Cobrança">${cobr}</td>
  ...
  function fillOfficeForm(office, readonly) {
    ...
    set("aoFieldTradeName", office?.trade_name);
    ...
    set("aoFieldAddress", office?.address);
    set("aoFieldCity", office?.city);
    set("aoFieldState", office?.state);
  ...
  function renderManagersSection() {
    ...
    let managers = svc().managersForOffice(global.__ampliState?.advocacyOfficeManagers || [], officeId);
    const q = String(_aoMgrSearchCarteira || "").trim().toLowerCase();
    if (q) {
      managers = managers.filter((m) => String(m.name || "").toLowerCase().includes(q));
    }
    const manage = canManage();
          ...
          .map((m) => {
            ...
            const acts = manage
              ? `<button type="button" class="secondary" data-ao-mgr-edit="${esc(m.id)}">Editar</button>
                 ...
                 <button type="button" class="secondary" data-ao-mgr-del="${esc(m.id)}">Excluir</button>`
              : "";
            return `<tr>
    ...
    host.innerHTML = `
      <div class="ao-mgr-head">
        <h4>Setor de Retomados — Gestores de Carteira — ${n}</h4>
      ...
      <div class="filter-bar" style="margin:0 0 10px">
        <label for="aoMgrSearchCarteira">Pesquisar gestor</label>
        <input id="aoMgrSearchCarteira" type="search" placeholder="Nome do gestor" value="${esc(_aoMgrSearchCarteira)}" />
      </div>
      <div class="table-wrap">
        <table class="table stacked">
          <thead><tr>
  ...
  function renderBillingManagersSection() {
    const host = document.getElementById("aoBillingManagersRoot");
    if (!host) return;
    const officeId = _editingId;
    const mgrSvc = global.partnerManagersService;
    const list = global.__ampliState?.partnerManagers || [];
    const manage = canManage() && !_readonly;
    if (!officeId) {
      host.innerHTML = `<p class="notice" style="margin:0">Salve o escritório para cadastrar gestores de cobrança.</p>`;
      return;
    }
    const rowsList = mgrSvc
      ? mgrSvc.listForParent(list, { advocacy_office_id: officeId }, "COBRANCA", _aoMgrSearchCobranca)
      : [];
    const n = rowsList.length;
    const addBtn = manage
      ? `<button type="button" class="secondary" id="aoAddBillingManager">+ Adicionar Gestor de Cobrança</button>`
      : "";
    const body = rowsList.length
      ? rowsList
          .map((m) => {
            const st = m.status === "INATIVO" ? '<span class="pc-status-inativo">Inativo</span>' : '<span class="pc-status-ativo">Ativo</span>';
            const tog = m.status === "INATIVO" ? "Reativar" : "Inativar";
            const acts = manage
              ? `<button type="button" class="secondary" data-ao-bill-edit="${esc(m.id)}">Editar</button>
                 <button type="button" class="secondary" data-ao-bill-toggle="${esc(m.id)}">${tog}</button>
                 <button type="button" class="secondary" data-ao-bill-del="${esc(m.id)}">Excluir</button>`
              : "—";
            return `<tr>
              <td data-label="Nome">${esc(m.name)}</td>
              <td data-label="Telefone">${esc(m.phone || "—")}</td>
              <td data-label="WhatsApp">${esc(m.whatsapp || "—")}</td>
              <td data-label="E-mail">${esc(m.email || "—")}</td>
              <td data-label="Cargo">${esc(m.role_title || "—")}</td>
              <td data-label="Status">${st}</td>
              <td data-label="Ações" class="actions">${acts}</td>
          ...
          .join("")
      : `<tr><td colspan="7" class="notice" style="text-align:center;padding:16px">Nenhum gestor de cobrança neste escritório.</td></tr>`;
    host.innerHTML = `
      <div class="ao-mgr-head">
        <h4>Financeiro — Gestores de Cobrança — ${n}</h4>
        ${addBtn}
      </div>
      <div class="filter-bar" style="margin:0 0 10px">
        <label for="aoMgrSearchCobranca">Pesquisar gestor</label>
        <input id="aoMgrSearchCobranca" type="search" placeholder="Nome do gestor" value="${esc(_aoMgrSearchCobranca)}" />
      </div>
      <div class="table-wrap">
        <table class="table stacked">
          <thead><tr><th>Nome</th><th>Telefone</th><th>WhatsApp</th><th>E-mail</th><th>Cargo/Função</th><th>Status</th><th>Ações</th></tr></thead>
          <tbody>${body}</tbody>
        </table>
      </div>`;
  }

  function fillManagerForm(manager, kind) {
    _aoMgrMode = kind === "COBRANCA" ? "COBRANCA" : "CARTEIRA";
    _editingManagerId = manager?.id || null;
    const title = document.getElementById("aoManagerModalTitle");
    const isCobranca = _aoMgrMode === "COBRANCA";
    if (title) {
      title.textContent = manager?.id
        ? isCobranca
    ...
    set("aoMgrWhatsapp", manager?.whatsapp);
    ...
    if (st) {
      const inactive = isCobranca ? manager && manager.status === "INATIVO" : manager && manager.active === false;
      st.value = inactive ? "INATIVO" : "ATIVO";
    }
  ...
  function readManagerForm() {
    return svc().normalizeManagerPayload({
      ...
      whatsapp: document.getElementById("aoMgrWhatsapp")?.value,
  ...
  function readOfficeForm() {
    return svc().normalizeOfficePayload({
      ...
      trade_name: document.getElementById("aoFieldTradeName")?.value,
      address: document.getElementById("aoFieldAddress")?.value,
      city: document.getElementById("aoFieldCity")?.value,
      state: document.getElementById("aoFieldState")?.value,
  ...
  function bindOnce() {
    ...
    document.getElementById("aoManagersRoot")?.addEventListener("input", (e) => {
      if (e.target.id === "aoMgrSearchCarteira") {
        _aoMgrSearchCarteira = e.target.value || "";
        renderManagersSection();
        const el = document.getElementById("aoMgrSearchCarteira");
        if (el) {
          el.focus();
    ...
    document.getElementById("aoBillingManagersRoot")?.addEventListener("click", async (e) => {
      if (e.target.closest("#aoAddBillingManager")) {
        if (!canManage()) return;
        if (!_editingId) {
          alert("Salve o escritório antes de adicionar gestores.");
          return;
        }
        fillManagerForm(null, "COBRANCA");
        return;
      }
      const mgrSvc = global.partnerManagersService;
      const list = global.__ampliState?.partnerManagers || [];
      const edit = e.target.closest("[data-ao-bill-edit]");
      const tog = e.target.closest("[data-ao-bill-toggle]");
      const del = e.target.closest("[data-ao-bill-del]");
      if (edit && mgrSvc) {
        const m = mgrSvc
          .listForParent(list, { advocacy_office_id: _editingId }, "COBRANCA")
          .find((x) => String(x.id) === edit.getAttribute("data-ao-bill-edit"));
        if (m) fillManagerForm(m, "COBRANCA");
      }
      if (tog && typeof global.togglePartnerManager === "function") {
        await global.togglePartnerManager(tog.getAttribute("data-ao-bill-toggle"));
        renderBillingManagersSection();
      }
      if (del && typeof global.deletePartnerManager === "function") {
        await global.deletePartnerManager(del.getAttribute("data-ao-bill-del"));
        renderBillingManagersSection();
      }
    });
    document.getElementById("aoBillingManagersRoot")?.addEventListener("input", (e) => {
      if (e.target.id === "aoMgrSearchCobranca") {
        _aoMgrSearchCobranca = e.target.value || "";
        renderBillingManagersSection();
        const el = document.getElementById("aoMgrSearchCobranca");
        if (el) {
          el.focus();
          const n = el.value.length;
          el.setSelectionRange(n, n);
    ...
    document.getElementById("aoManagerForm")?.addEventListener("submit", async (e) => {
      ...
      if (_aoMgrMode === "COBRANCA") {
        const mgrSvc = global.partnerManagersService;
        if (!mgrSvc || typeof global.savePartnerManager !== "function") return;
        const row = mgrSvc.normalizeManagerPayload(
          Object.assign({}, payload, {
            kind: "COBRANCA",
            status: payload.active === false ? "INATIVO" : "ATIVO",
          })
        );
        const errorsC = mgrSvc.validateManager(
          row,
          global.__ampliState?.partnerManagers || [],
          { advocacy_office_id: _editingId },
          _editingManagerId
        );
        const errBoxC = document.getElementById("aoManagerFormErrors");
        if (errorsC.length) {
          if (errBoxC) {
            errBoxC.classList.remove("hidden");
            errBoxC.innerHTML = `<ul>${errorsC.map((x) => `<li>${esc(x)}</li>`).join("")}</ul>`;
          }
          return;
        }
        const okC = await global.savePartnerManager(
          { advocacy_office_id: _editingId },
          _editingManagerId,
          row
        );
        if (okC) {
          closeManagerModal();
          renderBillingManagersSection();
          showOfficeNotice("Gestor de cobrança salvo neste escritório.");
        }
        return;
      }
